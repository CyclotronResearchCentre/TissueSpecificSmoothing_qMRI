...
paths = aj_default_GLM();

% Choose the smoothing approach
smoo_approachs = {'TWsmoot', 'TWS', 'TSPOON', 'SUSAN'};
smoo = 4;

% Select the jobfile for GLM
jobfile = {fullfile(paths.script_dir, 'batchSPMMReg_ageNonLinear_job.m')};
...
elseif smoo==4
    smoo_dir = fullfile(paths.ds_dir,'derivatives','AJ-SUSAN_unzipped');
else
...
if smoo==3
    aj_rename_gtspoon(smoo_dir); % Function to rename GTSPOON files if needed
end
...
for i = 1:length(qmetrics)
    inputs  = cell(3,1);
    
    for ii = 1:length(TCs)
        % OUTPUT Folder
        inputs{1} = cellstr(fullfile(paths.ds_dir,'derivatives',sprintf('AJ-%s_GLM_%s_age_nonlinear',smoo_approachs{smoo}, rft_hyps{hyp}),sprintf('%s_%s',qmetrics{i},TCs{ii})));
        ...
        inputs{2} = aj_select_smoofiles(smoo, smoo_dir, smoo_approachs, qmetrics{i}, TCs{ii});
